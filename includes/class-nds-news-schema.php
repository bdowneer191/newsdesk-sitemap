...
class NDS_News_Schema {
	 ...
	 *
	 * @since    1.0.0
	 * @param    array    $posts    Array of post data ready for XML conversion.
	 * @return   string             The complete, formatted XML string.
	 */
	public function build_sitemap( $posts ) {
		$xml = new DOMDocument( '1.0', 'UTF-8' );
		$xml->formatOutput = true;

		// Create urlset root element with all required namespaces
		...
		// Process each post into a <url> entry
	...
	/**
	 * Helper to create a single <url> entry
	 *
	 * @since    1.0.0
	 * @access   private
	 */
	private function create_url_element( $xml, $data ) {
		...
		// <image:image> block (if images enabled and present)
	...
	/**
	 * Helper to create the <news:news> segment
	 *
	 *
	 * @since    1.0.0
	 * @access   private
	...
	/**
	 * Helper to create the <image:image> segment
	 *
	 * @since    1.0.0
	 * @access   private
	 */
	...
	/**
	 * Validate XML against Google's News Sitemap Schema
	 *
	 * @since    1.0.0
	 */
		...
		if ( ! empty( $errors ) ) {
			...
			foreach ( $errors as $error ) {
				$messages[] = sprintf(
					...
					esc_html__( 'Line %1$d: %2$s', 'newsdesk-sitemap' ),
					$error->line,
					trim( $error->message )
				);
			}
