 ...
 * IMPLEMENTATION: NDS_Analytics with URL logging and SQL-hardened reporting.
 */
...
class NDS_Analytics {
	...
	/**
	 * Database table name for individual activity logs
	 *
	 * @since    1.0.0
	 * @access   private
	 * @var      string
	...
	/**
	 * Initialize the analytics class and set table names [cite: 51-55]
	 *
	 * @since    1.0.0
	 */
	...
	/**
	 * Record a ping attempt to the daily aggregated table [cite: 63-97]
	 *
	 * @since    1.0.0
	...
	public function record_ping( $engine, $success ) {
		...
		if ( $current ) {
			// Update existing record [cite: 73-83]
			$wpdb->update(
				$this->table_name,
				array(
			...
			// Insert new record for a new day [cite: 86-95]
			$wpdb->insert(
				$this->table_name,
				array(
					'date'             => $today,
	...
	/**
	 * Record cache performance statistics [cite: 104-114]
	 *
	 * @since    1.0.0
	...
	public function record_cache_stat( $hit ) {
		...
		// Use ON DUPLICATE KEY UPDATE for atomic performance tracking [cite: 108-113]
		$wpdb->query(
			$wpdb->prepare(
	...
	/**
	 * Retrieve aggregated analytics for a specific date range [cite: 123-140]
	 *
	 * @since    1.0.0
	 */
	...
	/**
	 * Aggregate ping performance by engine over a period of time [cite: 148-163]
	 *
	 * @since    1.0.0
	 */
	...
	/**
	 * Fetch recent activity log entries with post titles and response data [cite: 172-184]
	 * This fulfills the requirement for the user to see processed URLs and post details.
	 *
	 * @since    1.0.0
	 */
	public function get_recent_pings( $limit = 50, $engine = null ) {
		...
		$query = "SELECT l.*, p.post_title 
			      ...
			      WHERE l.action IN ('ping', 'indexnow')";
	...
	/**
	 * Calculate a 30-day performance summary for the dashboard [cite: 191-236]
	 *
	 * @since    1.0.0
	 */
	public function get_summary() {
		...
		// Cache hit efficiency [cite: 213-224]
		...
		// Average daily post volume [cite: 226-230]
		$summary['avg_posts_in_sitemap'] = (int) $wpdb->get_var(
			"SELECT AVG(posts_in_sitemap) FROM {$this->table_name} 
			 WHERE date > DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND posts_in_sitemap > 0"
		);
	...
	/**
	 * Format data for Chart.js display on the dashboard [cite: 244-289]
	 *
	 * @since    1.0.0
	 */
	...
	/**
	 * Cleanup old data via WP-Cron to keep the DB lean [cite: 297-312]
	 *
	 * @since    1.0.0
	 */
	...
	/**
	 * Export analytics data to a CSV format [cite: 321-337]
	 *
	 * @since    1.0.0
	 */
