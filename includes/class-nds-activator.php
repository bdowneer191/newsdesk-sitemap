...
class NDS_Activator {
	...
	public static function activate() {
		NDS_Logger::log( 'Activator::activate() called.' );

		// Create database tables
		NDS_Logger::log( 'Creating tables...' );
		self::create_tables();

		// Set default options
		NDS_Logger::log( 'Setting default options...' );
		self::set_default_options();

		// Create necessary directories
		NDS_Logger::log( 'Creating directories...' );
		self::create_directories();

		// Schedule cron events
		NDS_Logger::log( 'Scheduling events...' );
		self::schedule_events();

		// Flush rewrite rules to register our custom endpoints
		NDS_Logger::log( 'Flushing rewrite rules...' );
		flush_rewrite_rules();

		// Log activation
		self::log_activation();
        NDS_Logger::log( 'Activation sequence finished.' );
	}
