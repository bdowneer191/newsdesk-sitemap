...
class NDS_Sitemap_Generator {
	...
	private function generate_sitemap_index() {
		...
		if ( false !== $cached && ! isset( $_GET['nocache'] ) ) {
			echo $cached;
			return;
		}
		...
		$total_pages = (int) max( 1, ceil( $total_posts / $max_urls ) );
	...
	private function get_sitemap_posts( $page = 1 ) {
		...
		// Handle category and tag exclusions
		...
		if ( ! empty( $excluded_tags ) ) {
			$args['tag__not_in'] = array_map( 'absint', $excluded_tags );
		}
	...
	/**
	 * Helper to get the full sitemap URL
	 */
